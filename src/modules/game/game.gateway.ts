...
import { MagichanismService } from '../magichanism/magichanism.service';
...
async handleBuzzerAnswer(
  ...
  try {
    ...
    if (result.isCorrect) {
      this.dmxService.triggerFlash();
      this.audioService.playSoundEffect('correct');
      this.magichanismService.trigger(payload.tenantId, 'scene.correct_answer', { playerId: payload.playerId });
