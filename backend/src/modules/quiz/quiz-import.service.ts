...
export class QuizImportService {
  ...
  async importFromJson(fileBuffer: Buffer, userId: string) {
    let data: any;
    try {
    ...
    return this.quizService.create(data, userId);
  }

  async importFromCsv(fileBuffer: Buffer, userId: string) {
    let rows: any[];
    try {
    ...
    return this.quizService.create(quizPayload as any, userId);
  }
}